...
//WENN VERSION == omp wird die äußere Paralelisierung gesetzt. Ansonsten ist das Program seriell
  ...
  #define OUTERFOR
#endif
//Wenn VERSION == columns teilen sich die Threads die Spalten untereinander auf. Sonst die Zeilen
#if VERSION == columns
  #define INDEX1 j
  #define INDEX2 i
#else
  ...
  #define INDEX2 j
#endif
//Wenn VERSION == nested werden auch für die inneren for-Schleifen Threads gespornt, ergo werden die Daten elementweise aufgeteilt
#if VERSION == nested
  #define NESTED omp_set_nested(1);
  #define INNERFOR #pragma omp for private(j,star,residuum)
#else
  #define NESTED
  #define INNERFOR
#endif
//Wenn wir auch die Scheduling-Aufgabe fertig machen, können wir wohl diese definitionen brauchen
#if VERSION == static
  #define NOTINUSE
  #define NOTINUSE2
#else
...
calculate (struct calculation_arguments const* arguments, struct calculation_results *results, struct options const* options)
{
	...
	while (term_iteration > 0)
	{
		...
		for (INDEX1 = 1; INDEX1 < N; INDEX1++)
		{
			...
			if (options->inf_func == FUNC_FPISIN)
			{
				fpisin_i = fpisin * sin(pih * (double)INDEX1);
			}
			...
			for (INDEX2 = 1; INDEX2 < N; INDEX2++)
			{
				...
				if (options->inf_func == FUNC_FPISIN)
				{
					star += fpisin_i * sin(pih * (double)INDEX2);
				}
